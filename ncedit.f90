...
program ncedit
  ...
  integer :: i, j, k, t, l, ipoint, nx, ny
...
!  if(debug_level.ge.200) print *, "  tmax         = ", tmax
  ...
  case (4)
     ...
     case ('maxrain','averain','apw','apm','aps','ape')
        ...
        jsta = jmax/2 - 10 ! which is reffered to Y = -10 km in the Y cordinate
        jend = jmax/2 + 10 ! which is reffered to Y = +10 km in the Y cordinate
...
!        iend = imax/2 - 39 ! which is reffered to X = -40 km in the X cordinate
        ista = imax/2 - 200 ! which is reffered to X = -200 km in the X cordinate
        iend = imax/2 - 149 ! which is reffered to X = -150 km in the X cordinate
        allocate( var_in(imax,jmax,1,1) ) ! xy + z-loop
        istart = (/ 1, 1, 1, 1 /)
        icount = (/ imax, jmax, 1, 1 /)
        var_in(1:imax,1:jmax,1,1) = nan
     ...
     case ('vthetaeca','vthetavca')
        ! An area-averaged value of the vertical profile of theta-e, and virtucal theta in the convective area
        inx = imax
        iny = jmax
        inz = 1
        int = 1
        ...
        tmp4(1:imax,1:jmax) = 0.
     case ('maul')
        ! To check whether MAUL (Moist Absolutely Unstable Layers) exists or not within the convective area
        inx = imax
        iny = jmax
        inz = 1
        int = 1
     ...
     case ('tcapeave3d','tcinave3d','tlfcave3d','tlnbave3d')
        ! for calculation of CAPE using getcape for a 3-D output
        inx = imax
        iny = jmax
        inz = kmax
        int = 1
        allocate( var_in(imax,jmax,kmax,1) ) ! xyz + t-loop
        istart = (/ 1, 1, 1, 1 /)            ! this value will be replaced during t-loop
        icount = (/ imax, jmax, kmax, 1 /)
        var_in(1:imax,1:jmax,1:kmax,1) = nan
        allocate( tmpc1(imax,jmax,kmax),tmpc2(imax,jmax,kmax),tmpc3(imax,jmax,kmax) )
        tmpc1(1:imax,1:jmax,1:kmax) = 0.
        tmpc2(1:imax,1:jmax,1:kmax) = 0.
        tmpc3(1:imax,1:jmax,1:kmax) = 0.
        allocate( tmp(tmax,1) )
        tmp(1:tmax,1) = 0.
        allocate( tmp1(imax,jmax),tmp2(imax,jmax),tmp3(imax,jmax),tmp4(imax,jmax) )
        tmp1(1:imax,1:jmax) = 0.
...
!           if( (tmp1(i,j).gt.0.1).and.(tmp2(i,j).gt.0.01) ) then
  ...
  case ('vthetavca')
     ! A area-averaged vertical profile of virtual potential temperature in the convective area
     ! The horizontal averaging depends on ista and iend
     ! *** this section work with flag = 4 ***
     if(flag.ne.4) then
        print *, "WARNING: flag = ", flag, "is under construction for now..."
...
!           tmp4(i,j) = TqvP_2_thetav( tmp0, tmp3(i,j), tmp1(i,j) )
           ...
           tmp4(i,j) = tmp2(i,j)*( 1+0.61*tmp3(i,j) )
        end do
        end do
        tmp1 = 0.
        ! --- read winterp [m/s]
...
!           if( (tmp1(i,j).gt.0.1).and.(tmp2(i,j).gt.0.01) ) then
        ...
        print '(a24,i3,i6,f8.2,f10.2)', " k,ipoint,z,thetavca = ", k,ipoint,z(k),tmp(k,1)
     end do ! end of k-loop
...
!           tmp4(i,j) = TqvP_2_thetav( tmp0, tmp3(i,j), tmp1(i,j) )
  ...
  case ('maul')
     ! To check whether MAULs exist or not within the convective area
     ! The horizontal averaging depends on ista and iend
     ! *** this section work with flag = 4 ***
     if(flag.ne.4) then
        print *, "WARNING: flag = ", flag, "is under construction for now..."
     ...
     do k = 1, kmax, 1
        ...
        do l = 1, 11, 1
           ...
           do j = 1, jmax, 1
           do i = ista, iend, 1
              tmp(k,l) = tmp(k,l) + tmpi(i,j,l)
           end do
           end do
           tmp(k,l) = tmp(k,l)/real((iend-ista+1)*jmax)
        end do
...
!      end do
  ...
  if((flag.eq.4).or.(flag.eq.8)) then
     ...
     case ('maxrain','averain','apw','apm','aps','ape',         &
        ...
        !
     case ('tcapeave3d','tcinave3d')
        do t = 1, tmax, 1
           write(20,111) real(time_in(t)/dble(60.)), tmp(t,1)
           if(debug_level.ge.200) print 222, "t,time,var = ", t, real(time_in(t)/dble(60.)), tmp(t,1)
        end do
        !
     case ('tlfcave','tlnbave','tlfcave3d','tlnbave3d')
        do t = 1, tmax, 1
     ...
     case ('maul','spmaul')
        ...
        do k = 1, kmax, 1
           ...
           if(debug_level.ge.200) print 224,                    &
             "  p,z,tc,td,rh,qv,wd,ws,th,the,thv = ", tmp(k,1), &
                tmp(k,2),tmp(k,3),tmp(k,4),tmp(k,5),tmp(k,6),   &
                tmp(k,7),tmp(k,8),tmp(k,9),tmp(k,10),tmp(k,11)
        end do
...
113  format(f7.1,f7.0,2f7.1,f7.0,f7.2,2f7.0,3f7.1)
222  format(a22,i5,f8.3,f18.8)
223  format(a22,i5,f8.3,f18.8)
224  format(a37,f7.1,f7.0,2f7.1,f7.0,f7.2,2f7.0,3f7.1)
...
!        ptd(k) = gettd(p2,t2,qv2)
      ...
      if( (b2.ge.0.0) .and. (b1.lt.0.0) )then
        ...
        cin  = cin  + narea
        narea = 0.0
        ! calculate LFC (for low-to-mid level inversion)
        zlfc = z(k-1)+(z(k)-z(k-1))*(0.0-b1)/(b2-b1)
        plfc = p(k-1)+(p(k)-p(k-1))*(0.0-b1)/(b2-b1)
      ...
      elseif( b2.lt.0.0 )then
        ...
        parea =  0.5*dz*(b1+b2)
        narea =  0.0
        ! calculate EL (for low-to-mid level inversion)
        zel = z(k-1)+(z(k)-z(k-1))*(0.0-b1)/(b2-b1)
        pel = p(k-1)+(p(k)-p(k-1))*(0.0-b1)/(b2-b1)
      endif
